...
import projectsWithTasksDueTodayAggregate from "./aggregations/projects.aggregate";
 ...
 */
const runProjectAggregate = async () => {
    try {
        const client = await MongoClient.connect(process.env.DATABASE_URL!);
        const db = client.db();

        const projects = await db.collection(PROJECTS)
            .aggregate(projectsWithTasksDueTodayAggregate)
            .toArray();
...
export default runProjectAggregate;
